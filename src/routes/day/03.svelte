<script>
	...
	const keys = [
		...whiteKeys.map((path) => ({ className: 'white-keys', path })),
		...blackKeys.map((path) => ({ className: 'black-keys', path }))
	];

	onMount(() => {
		audio = Array.from(Array(23)).map((_, i) => new Audio(`/audio/key-${i + 1}.mp3`));
	});
...
<div class="root">
	<label class="volume">Volume <input type="range" bind:value={volume} /></label>
	<svg
		width="1387"
		height="467"
